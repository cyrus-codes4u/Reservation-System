...
> This is a model reservation system for fine dining restaurants.
...
This project relies on two libraries [Knex](http://knexjs.org/) and Express.js. Additionally, knex functions are designed to interact with a PostgreSQL database
...
The `./back-end` folder contains all the code for the backend support.
...
| `./back-end/src/errors/methodNotAllowed.js`              | Defined an Express API "method not allowed" handler.                                                        	 |
...
| `./back-end/reservations/reservations.router.js`         | Defines the nested router to handle requests made to `/reservations` route.                             		 |
| `./back-end/reservations/reservations.controller.js`     | Validation middleware and response protocol for requests made to `/reservations` route.                             |
| `./back-end/reservations/reservations.service.js`        | Knex queries to remote database for requests made to `/reservations` route.                         		 |
| `./back-end/reservations/tables.router.js`   		   | Defines the nested router to handle requests made to `/tables` route.                             			 |
| `./back-end/reservations/tables.controller.js`           | Validation middleware and response protocol for requests made to `/tables` route.                                   |
| `./back-end/reservations/tables.service.js`              | Knex queries to remote database for requests made to `/tables` route.                     		        	 |
...
These commands allow for edits to be made within the `back-end/src/db/migrations` and `back-end/src/db/seeds` directories.
...
Test are split up by feature/page. You can run the tests for a given feature/page by running:

`npm run test:X` where `X` is a number 1-8.

Have a look at the following examples:

- `npm run test:1` runs all the tests for feature 1 (both frontend and backend).
- `npm run test:3:backend` runs only the backend tests for feature 3.
- `npm run test:3:frontend` runs only the frontend tests for feature 3.

Whenever possible, frontend tests will run before backend tests.
